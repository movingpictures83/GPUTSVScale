...
#include "GPUTSVScalePlugin.h"

void GPUTSVScalePlugin::input(std::string file) {
 inputfile = file;
...
printf("***Scale on %d x %d Matrix on GPU***\n",N,N);
multMatByValue<<<N,N>>>(val, pA, pC, N);
...
void GPUTSVScalePlugin::output(std::string file) {
...
PluginProxy<GPUTSVScalePlugin> GPUTSVScalePluginProxy = PluginProxy<GPUTSVScalePlugin>("GPUTSVScale", PluginManager::getInstance());
